...
birthweight_df %>% 
  ...
  ggplot(aes(x = mrace, y = pred)) + 
  geom_point(color = "red") +
  geom_point(aes(y = bwt), color = "blue")
  labs(
    title = " Birth Weight v.s. Mother's race prediction",
    x = 'Mother\'s race',
    y = 'Baby\'s Weight'
  )
```
...
cv_df = 
  ...
  mutate(
    ...
    mod_3  = map(train, ~lm(bwt ~ babysex + bhead + blength, data = .x))) %>% 
  mutate(
